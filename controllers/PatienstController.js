...
class PatiensController {
     ...
     */
    async store(req,res){
        //destructing object yang ada di request body
        const {name, phone, address, status} = req.body 
        //menvalidasi data harus diisi
        if(!name ||!phone ||!address ||!status){
            //membuat response pesan jika validasi tidak sesuai
            const data = {
                message : "All fields must be filled correctly"
            };
            //mengirim response jika data gagal divalidasi dengan kode 422
            return res.status(422).json(data)
        //validasi phone jika bukan angka    
        }else if(isNaN(phone)){
            //membuat response pesan jika validasi tidak sesuai
            const data = {
        ...
        //menggunakan method create untuk menambah data baru
    ...
    async update(req,res){
        ...
        //validasi data dan cek apakah datanya ada atau tidak
        if (patiens){
        //destructing object yang ada di request body
        const {name, phone, address, status} = req.body 
        //menvalidasi data harus diisi
